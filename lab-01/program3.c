...
// I've never bothered much with the CPP outside of includes and basic defines, so I guess we can
...
#define PROGRAM3_ALLOW_WRITELN_EMPTY_VARIADIC
    ...
    // Everyone's impersonating GCC, so ##__VA_ARGS__ is decently likely to work, but it's not
    // standard, just a GNUC extension
    #define writeln(s, ...) printf(s "\n", ##__VA_ARGS__)
#elif defined(PROGRAM3_FORCE_WRITELN_PRINTF)
    // __VA_ARGS__ has been around since C99
    // If you really wanna put writeln(format_string, NULL); everywhere just for printf, be my guest
    #define writeln(format_string, ...) printf(format_string "\n", __VA_ARGS__)
#else
    // I guess this is really the easiest solution, puts even adds a newline for you...
    #define writeln puts
#endif

// I /would/ put checks for C99 (and __VA_ARGS__ support), but that's when single-line comments
// were added, so there's not much use in pretending to support anything prior
