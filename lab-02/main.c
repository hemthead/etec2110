...
// Returns the number of iterations in the collatz conjecture required to reach
// a value of 1. Does not include the original number.
...
void collatz_print_sequence(long long int number) {
  if (number <= 0) {
    printf("Error: %lld <= 0, the Collatz Conjecture only applies to positive "
           "integers.\n",
           number);
    return;
  }
